...
Additionally, you do occasionally get pdfs which are already in text format (not image scans). In such cases, again,
normalizing first is best practice. You can do this with another utility I only recently realized was an option: *Ghostscript*.
With this commandline utility, you can "repair" pdfs---or in the case there's nothing technically invalid or corrupt about
them, you can standardize pdfs to a single specification:

+ gs -o &lt;filename&gt;-repaired.pdf -sDEVICE=pdfwrite -dPDFSETTINGS=/prepress &lt;filename&gt;.pdf

This is necessary because in practice, when obtaining pdfs "from the wild" you often get ones which were generated by a buggy
pdf maker. For example when you copy and paste (or "pdftotext" extract) data tables, with such buggy pdfs you will get the table
data but often all out of order, which becomes time consuming to put back in order (usually with Vim). I did not use ghostscript
in this project as part of my toolset primitives, but I wish I had. As stated above, I only recently realized it was a possibility.
