...
All the validators read in the data entry csv file. The "space" validator simply made sure each entry was separated by no more
than one empty line. The "entry line" validator looked at each entry line and made sure it conformed to a regular structure,
notably that each entry ended with a comma (except the final entry of the line) and that each entry was separated by one or more
...
and can be held up to scrutiny, but it's also important to verify the mappings were done as intended.
...
*least common multiple* translation of structure, we will have a raw and unified comma separated value text file.
...
As such, I could not rely on an outside sorter and used my own. This is a good example of the issues that can arise with
modularity and interoperability of code.
...
(factorizes) all other headers into a condensed, consolidated csv file.
...
As with the creation of the "folded\_map.log" in the *All headers* analysis, some of the terminal headers within "factor\_map.csv"
...
+ Note: If you take a look at the "terminal\_headers.log" generated by the "pull\_terminal\_headers" script, you'll notice
...
into a comma separated value file---this being important as the consolidation subphase assumes and makes use of the csv file
generated here.
...
R script, and the "pull\_bad\_cols" (without the ".r" extension) is a bash wrapper script to simplify and redirect its output.
...
policy makers, think-tanks, etc. use these numbers, keep in mind said numbers do not distinguish council and non-council varieties
of finance. If for example someone is interested in Council's median salary in terms of what they're paid for their role as Council,
the median provided here will be skewed to be higher than the numbers this person seeks.
...
I chose to create a separate "translation" phase even though it greatly extended the workload as part of the modular design
